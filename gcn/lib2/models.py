...
class base_model(object):
    ...
    def predict(self, data, labels=None, sess=None):
        ...
        for begin in range(0, size, self.batch_size):
            ...
            if labels is not None:
                ...
                batch_pred, batch_loss, batch_scores = sess.run([self.op_prediction, self.op_loss, self.op_probabilities], feed_dict)
                loss += batch_loss
            else:
                batch_pred = sess.run(self.op_prediction, feed_dict)
             
            predictions[begin:end] = batch_pred[:end-begin]
            scores[begin:end] = batch_scores[:end-begin]
            #for i in range(scores.shape[0]):
             #   print(sum(scores[i,:]))
        if labels is not None:
            return predictions, loss * self.batch_size / size, scores
        else:
            return predictions
    ...
    def evaluate(self, data, labels, sess=None):
        ...
        predictions, loss, y_scores = self.predict(data, labels, sess)
        ...
        return string, accuracy, f1, loss, y_scores
    ...
    def fit(self, train_data, train_labels, val_data, val_labels):
        ...
        for step in range(1, num_steps+1):
            ...
            if step % self.eval_frequency == 0 or step == num_steps:
                ...
                string, accuracy, f1, loss, score = self.evaluate(val_data, val_labels, sess)
        ...
        print('validation accuracy: peak = {:.2f}, mean = {:.2f}'.format(max(accuracies), np.mean(accuracies)))
        writer.close()
        sess.close()
        
        t_step = (time.time() - t_wall) / num_steps
        return accuracies, losses, t_step, y_scores
...
class cgcnn(base_model):
    ...
    def __init__(self, L, F, K, p, M, filter='chebyshev5', brelu='b1relu', pool='mpool1',
                ...
                regularization=0, dropout=0, batch_size=100, eval_frequency=10,
                dir_name=''):
        super().__init__()
